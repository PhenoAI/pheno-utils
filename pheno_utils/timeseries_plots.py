...
# %% auto 0
__all__ = ['TimeSeriesFigure', 'format_xticks', 'format_timeseries', 'plot_events_bars', 'plot_events_fill', 'prepare_events',
...
# TODO: rename to prepare_timeseries? prepare_timeseries_for_plotting?
def prepare_events(
    events: pd.DataFrame,
    x_start: str,
    x_end: str,
    ...
    events = format_timeseries(events, participant_id, array_index, time_range, x_start, x_end)

    # Filter events based on y_include and y_exclude
    events = events.dropna(subset=[x_start, x_end])
    if hue is not None and hue in events.index.names:
        events = events.reset_index(hue)
    if label is not None and label in events.index.names:
        events = events.reset_index(label)
    ...
    if hue in events.columns:
        colors = get_color_map(events, hue, palette)
    else:
        colors = None
